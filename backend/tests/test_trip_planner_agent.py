...
def install_fake_langchain_openai(monkeypatch, result: trip_planner_agent.PlannerDraft) -> tuple[type, type]:
    ...
    class FakeChatOpenAI:
        ...
        def __init__(self, **kwargs):
            """记录 Planner 初始化模型时使用的参数。

            Args:
                **kwargs: ChatOpenAI 初始化参数。
            """
            FakeChatOpenAI.last_init_kwargs = kwargs

        def invoke(self, messages):
            """记录提示消息并返回预设的结构化 Planner 结果。

            Args:
                messages: Planner 发送给模型的消息列表。

            Returns:
                FakeResponse: 包含预设 Planner JSON 的假响应。
            """
...
def test_collect_trip_context_calls_rag_tool_with_expected_arguments(monkeypatch) -> None:
    ...
    def fake_get_destination_guide_context(destination, preferences, pace, special_notes, top_k):
        """记录 RAG 工具入参并返回固定攻略上下文。

        Args:
            destination: 目的地名称。
            preferences: 用户偏好列表。
            pace: 行程节奏。
            special_notes: 用户补充要求。
            top_k: 召回片段数量。

        Returns:
            tuple: 固定上下文及三组零 Token 消耗统计。
        """
...
def test_generate_day_edit_draft_accepts_nested_day_shape(monkeypatch) -> None:
    ...
    class FakeChatOpenAI:
        ...
        def invoke(self, messages):
            """返回采用 DayPlan 嵌套形态的预设编辑结果。

            Args:
                messages: 单日编辑发送给模型的消息列表。

            Returns:
                FakeResponse: 包含嵌套编辑 JSON 的假响应。
            """
            return FakeResponse(
                json.dumps(
                    {
